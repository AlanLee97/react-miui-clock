...
export default function NumberSlideSelector(props = {}) {
  const {start = 0, end = 23, width = 100, unit = '', onSelected = () => {}} = props;
  const [list, setList] = useState([]);
  const [reachEnd, setReachEnd] = useState(true);
  const [scrollFinsh, setScrollFinsh] = useState(true);
  const [curIndex, setCurIndex] = useState(0);
  let [scrollTimer, setScrollTimer] = useState(null);
  let [tapping, setTapping] = useState(false);
  ...
  const onTouchEnd = e => {
    setTapping(false);
    if(disableOnScroll) return;
    let el = slideWrapperRef.current
